...
void LaserSlamWorker::publishTrajectory(const Trajectory& trajectory,
                                        const ros::Publisher& publisher, const ros::Publisher& pose_publisher) const {
  ...
  std::vector<geometry_msgs::PoseStamped> myposes;
  ...
  for (const auto& timePose : trajectory) {
    ...
    // itt pedig publisholom
    //pose_publisher.publish(pose_msg);
    traj_msg.poses.push_back(pose_msg);
     
    myposes.push_back(pose_msg);
  }

  if(delay_counter>35){
	for(int i=0; i<myposes.size(); i++){
		pose_publisher.publish(myposes.at(i));	
	}
	delay_counter=0;
  }
...
void LaserSlamWorker::publishTrajectories() {
  ...
  publishTrajectory(trajectory, trajectory_pub_, trajectory_poses_pub_);
}
